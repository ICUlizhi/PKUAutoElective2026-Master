...
class ElectiveClient(BaseClient):
    ...
    def get_ElectSupplement(self, href, **kwargs):
        """ 补选一门课 """
        return self.do_select_action(href, **kwargs)

    def do_select_action(self, href, **kwargs):
        """ 通用选课动作（补选/候补/预选等） """

        if not href:
            raise RuntimeError("empty href for select action")
        if not href.startswith("/elective2008/"):
            raise RuntimeError(
                "Unexpected href for select action: %r. "
                "Expected href starts with '/elective2008/'." % href
            )
